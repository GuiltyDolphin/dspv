...
testGroup("NestMap",
    ...
    testGroup("get",
        testGet("top level key, when there are no subkeys under this key", m0, ["k1"], 1),
        testGet("top level key, when there are subkeys under this key", m1, ["k1"], 1),
        testGet("subkey, when there are higher keys", m1, ["k1", "k2"], 2),
    ...
    testGroup("getBestAndRest",
        testGetBestAndRest("top level key, when there are no subkeys under this key", m0, ["k1"], [1, []]),
        testGetBestAndRest("top level key, when there are subkeys under this key", m1, ["k1"], [1, []]),
        testGetBestAndRest("top level key, when there are no subsequent keys", m0, ["k1", "k2"], [1, ["k2"]]),
        testGetBestAndRest("subkey, when there are higher keys", m1, ["k1", "k2"], [2, []]),
        testGetBestAndRest("subkey, when there are no higher keys", m2, ["k1", "k2"], [2, []]),
        testGetBestAndRest("subkey, when there are no subsquent keys", m2, ["k1", "k2", "k3"], [2, ["k3"]]),
        testGetBestAndRestNotThere("top level key that doesn't exist", m0, ["k"]),
        testGetBestAndRestNotThere("subkey that doesn't exist when toplevel key exists", m1, ["k1", "k3"]),
    ),
).runAsMain();
